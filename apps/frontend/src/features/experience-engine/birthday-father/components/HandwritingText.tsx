...
export const HandwritingText = memo(function HandwritingText({
  ...
  useEffect(() => {
    ...
    // Prevent replaying the letter when parent re-renders (e.g. CTA state)
  ...
  return (
    <div className={`fb-paper-text ${className}`}>
      {lines.map((line, i) => {
        ...
        return (
          <span
            key={`line-${i}`}
            className={`fb-paper-line ${count > 0 ? 'writing' : ''} ${count >= line.length ? 'done' : ''}`}
          >
            {shown}
            {isActive && <span className="fb-cursor" aria-hidden="true" />}
          </span>
        );
      })}
    </div>
